  ...
  <body>
    ...
    <div class="x-body">
      ...
      <table class="layui-table">
        ...
        <tbody>
          <c:forEach var="result" items="${allResult}">
          <tr>
            <td>${result.sNo}</td>
            <td>${result.sName}</td>
            <td>${result.aName}</td>
            ...
            <td>${result.phone}</td>
            <td class="td-manage">
	            <c:if test="${result.state=='通过'}">
	              <button class="layui-btn layui-btn-normal"  type="button">
					${result.state}
	              </button>
	            </c:if>
	            <c:if test="${result.state=='未通过'}">
      ...
      <div class="page">
        <div>
		    <c:if test="${indexNum!=1}">
		      <a href="${ctx}/examine/getAllResult?page=${indexNum-1}" class="prev" href="">&lt;&lt;</a>
		    </c:if>
		    
		      <span class="current"> ${indexNum}</span>
		      <c:if test="${indexNum<=sum}">
		      	<a class="next" href="${ctx}/examine/getAllResult?page=${indexNum+1}">&gt;&gt;</a>
		      </c:if>
