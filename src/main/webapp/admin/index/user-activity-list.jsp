...
<div class="x-body">
  <div class="layui-row">
    <form action="${ctx}/activity/userSearchActivity" method="post" class="layui-form layui-col-md12 x-so">
    ...
    <c:forEach var="act" items="${activityList}">
      <tr>
	    ...
	    <c:if test="${indexNum!=1}">
	      <a href="${ctx}/activity/userListAll?page=${indexNum-1}" class="prev" href="">&lt;&lt;</a>
	    </c:if>
	   
	      <span class="current"> ${indexNum}</span>
	     <c:if test="${indexNum<=sum}">
	       <a class="next" href="${ctx}/activity/userListAll?page=${indexNum+1}">&gt;&gt;</a>   
	    </c:if>
